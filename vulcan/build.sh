#!/usr/bin/env bash
# Build libvulcan (if missing) and the CBL policy .so.
#
# Usage:
...
echo ">>> building cbl_policy.so (evolve block: $EVOLVE_SRC)"
g++ -std=c++20 -O2 -shared -fPIC \
  ...
  "$HERE/cbl_policy.cpp" \
  -o "$HERE/cbl_policy.so"

echo "Built: $HERE/cbl_policy.so"
