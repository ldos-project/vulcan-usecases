#!/bin/bash
# Evaluate the GapBS (BC) benchmark on Cloudlab and collect results.
# Arguments:
# 1: Host name
...
# Run the GapBS (BC) benchmark and collect results.
echo "Running GapBS BC benchmark on $HOST ..."
ssh -o StrictHostKeyChecking=no $HOST "tmux new-session -d -s gapbs_benchmark \"
    rm -f \$HOME/log.txt &&
    /usr/bin/time -v numactl -N0 sudo DRAMSIZE=1558183936 NVMSIZE=21078474752 LD_PRELOAD=\$HOME/tiering_solutions/src/libarms.so OMP_NUM_THREADS=16 MIN_INTERPOSE_MEM_SIZE=134217728 /mnt/data/workloads/gapbs/bc -n 16 -f /mnt/data/inputs/twitter.sg > \$HOME/log.txt 2>&1
    \""
